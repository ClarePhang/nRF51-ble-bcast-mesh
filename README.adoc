...
Bluetooth Low Energy based rebroadcasting mesh implementation on the nRF52 called *nRF OpenMesh*.
Works with SoftDevice S132 v2.0.0  with Timeslot API, and the nRF52 SDK 11.0.0. It
Offers an API for implementing your own mesh enabled application, operating
concurrently with regular BLE applications.

== Usage

Right now this branch has only one example project named BLE_Gateway intended for nRF52 application.This can be found under
_nRF52/examples/_. This may be used as a basis for what modifications are neeed to migrate from nRF51 to nRF52.In theory, the framework should be compatible with most SoftDevice based projects, but some restrictions to
hardware and software modules apply, see link:docs/how_it_works.adoc#resource-allocation[Resource allocation]  for
...
framework. This allows you to operate an Arduino along with an nRF52 dev kit
in the mesh, making it easier to create mesh applications. Each nRF52 example
project has a separate target (named "nRF52832 xxAA serial") that incorporates
the serial interface with the rest of the project. 

=== Getting started

The framework requires nRF52 SDK 11.0.0 and can be built with both Keil uVision
...
Clone this repo into _<SDK11-folder>/examples/<Project-folder>_.
...
projects.It is also compatible with the SDK module
...
The framework resides on top of the nRF52 SoftDevice, utilizing the Timeslot
